...
pub fn run(args: &Args) -> CliResult<()> {
    ...
    if args.flag_select.is_empty() && args.flag_group.is_empty() {
        return Err(CliError::Other(
            "fill requires either --select or --group".into(),
        ));
    }
    ...
    let cols: Vec<usize> = if !args.flag_select.is_empty() {
        ...
        (0..rows[0].len()).collect()
    };

    let _group_cols: Vec<usize> = if !args.flag_group.is_empty() {
        let gh_str = args.flag_group.join(",");
        let gcols = SelectColumns::from_str(&gh_str).map_err(CliError::Other)?;
        let gsel = gcols
            .selection(&headers, !rconfig.no_headers)
    ...
    if args.flag_backfill {
        for i in (0..rows.len()).rev() {
            for &c in &cols {
                if rows[i].get(c).map(|f| f.is_empty()).unwrap_or(true) {
                    if let Some(next) = (i + 1..rows.len())
                        .find(|&j| !rows[j].get(c).map(|f| f.is_empty()).unwrap_or(true))
                    {
        ...
        for row in rows.iter_mut() {
            for &c in &cols {
                if row.get(c).map(|f| f.is_empty()).unwrap_or(true) {
                    if let Some(ref val) = last_val[c] {
                        row.push_field(val);
                    }
                } else {
                    last_val[c] = Some(row[c].to_vec());
                }
            }
        }
    }
