...
/// the splace to insert the part of the filename generated by `dsv`.
#[derive(Clone, Debug)]
...
impl FilenameTemplate {
        ...
        P: AsRef<Path>,
    {
        ...
        if let Some(parent) = full_path.parent() {
            // We may be called concurrently, especially by parallel `dsv
...
impl FromStr for FilenameTemplate {
    type Err = String;
    fn from_str(s: &str) -> Result<FilenameTemplate, String> {
        ...
        if chunks.len() == 2 {
            ...
            Err("The --filename argument must contain one '{}'.".to_owned())
        }
    }
}
